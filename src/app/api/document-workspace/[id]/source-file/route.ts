...
export async function GET(
  ...
  try {
    ...
    if (job?.type !== 'office') {
      return NextResponse.json(
        { error: 'Source file is only available for Document Workspace projects' },
        { status: 400 }
      );
    }
    ...
    if (!filePath || typeof filePath !== 'string') {
      return NextResponse.json(
        { error: 'Source file is not available' },
        { status: 404 }
      );
    }
    ...
    if (!filePath.startsWith(expectedPrefix)) {
      console.error('[Document Workspace Source Download] Stored path failed ownership check', {
        jobId: id,
        userId: job.userId,
        filePath,
      });
    ...
    const filename = job.originalFilename || job.filename || metadata.name?.split('/').pop() || 'source-file';
    ...
    console.error('[Document Workspace Source Download] Error:', error);
